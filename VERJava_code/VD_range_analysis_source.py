...
def analysis_patch_exist_version(path_json, target_repo, CVE_ID, zip_list):
    ...
    for one_version_file in zip_list:
        ...
        patch_list = []
        for parent, dirnames, filenames in os.walk(one_version_file):
            for filename in filenames:
                if filename.find(file_name)!=-1:
                    patch_list.append(parent +'/'+ filename)
        ...
        for patch_file in patch_list:
            ...
            if os.path.exists(patch_file):
                ...
                if patch_exist == False and vul_exist == True:
                    if this_version not in result_version:
                ...
                if func_exist == False and patch_func != 'no':
                    if this_version not in file_or_func_no_exist:
